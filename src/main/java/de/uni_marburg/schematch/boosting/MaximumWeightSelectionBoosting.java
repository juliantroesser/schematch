...
public class MaximumWeightSelectionBoosting implements SimMatrixBoosting{

    @Override
    public float[][] run(MatchTask matchTask, SimMatrixBoostingStep matchStep, float[][] simMatrix) {

        float[][] squareSimMatrix = makeSquareMatrix(simMatrix);
        float[][] squareMatrixForMaximization = transformForMaximization(squareSimMatrix);

        HungarianAlgorithm hungarianAlgorithm = new HungarianAlgorithm(squareMatrixForMaximization);
        int[][] indicesForMaximumWeightMatching = hungarianAlgorithm.findOptimalAssignment();

        return setEntriesToOne(indicesForMaximumWeightMatching, simMatrix);
    }
    ...
    public static class HungarianAlgorithm {
        ...
        public HungarianAlgorithm(float[][] matrix) {
    ...
    public float[][] makeSquareMatrix(float[][] costMatrix) {
    ...
    public float[][] transformForMaximization(float[][] costMatrix) {
        ...
        // Transform the cost matrix
        int rows = costMatrix.length;
        int cols = costMatrix[0].length;
        float[][] transformedMatrix = new float[rows][cols];

        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                transformedMatrix[i][j] = maxValue - costMatrix[i][j];
            }
        }
    ...
    public float[][] setEntriesToOne(int[][] indices, float[][] matrix) {

        // Get the number of rows and columns of the matrix
        int numRows = matrix.length;
        int numCols = matrix[0].length;

        // Create a new matrix with the same dimensions as the original matrix, initialized to 0
        ...
        for (int[] index : indices) {
            ...
            if (row >= 0 && row < numRows && col >= 0 && col < numCols) {
                // Set the corresponding entry in the result matrix to 1
                result[row][col] = 1.0f;
            }
        }
